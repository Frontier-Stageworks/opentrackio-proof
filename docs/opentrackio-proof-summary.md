...
**Claim scope.** Both theorems above compare OpenCV's forward-distortion formula against the *same* formula shape — undistorted normalised coordinate in, distorted coordinate out — applied to the converted `l`/`q` coefficients at the scaled screen point. This is a same-direction coordinate-conjugacy result, related by the coordinate scaling `S_F(x,y) = (F·x, F·y)`. It is not a claim about OpenTrackIO's *native* undistortion consumption of those coefficients: the OpenTrackIO JSON schema's `distortion.model` field defaults to `"Brown-Conrady D-U"` — distorted input, undistorted output — the opposite direction from what is proved here. Whether a real OpenTrackIO consumer that treats the converted coefficients as native D→U OpenLensIO coefficients would reproduce correct undistortion is a separate, harder, currently unproven claim, requiring either an exact inverse-function bridge or a bounded-error approximation theorem. See `docs/specification-questions.md` (SQ-CV-07) and `docs/limitations.md`.
...
### 4.3 Investigation: A Suspected Tangential Conversion Error

The theorems above formalize the paper's tangential consistency condition exactly as stated: `δx_cv = δx_oti`, i.e. the OpenCV and OpenTrackIO tangential displacements are equated directly, forcing `q1 = p1/F², q2 = p2/F²`. A separate investigation, not part of the formalization above and not modifying any theorem in it, asks whether that stated condition is itself physically consistent with the paper's own coordinate map.

The paper defines the distorted-point conversion as `ε'_x,d = F·x''`. Writing both sides as undistorted coordinate plus displacement (`x'' = x' + δx_cv`, `ε'_x,d = ε_x + δx_oti`, `ε_x = F·x'`) gives `δx_oti = F·δx_cv` — the tangential displacement, being additive, should pick up one factor of F on conversion. This differs from the paper's stated equation, which has zero factors of F. (The radial term needs no analogous fix: it is multiplicative, `x'·R(r)`, so the F is already carried through the coordinate itself.)

Re-deriving the conversion under `F·δx_cv = δx_oti` gives `q1 = p1/F, q2 = p2/F` — one power of F, not two — proved in `DistortionConversionCorrected.lean` as direct analogs of the Layer 1/2 theorems above. The pixel-level consequence, proved in `Pipeline/PixelIffCorrected.lean`, is the more striking finding: substituting the corrected `q1, q2` into the full pipeline theorem, the `ws/w = fx` condition required by `opencv_openlensio_full_pipeline_pixel_iff` does not survive — full pixel agreement holds unconditionally. A companion theorem exhibits a concrete case with `ws/w ≠ fx` where pixel agreement still holds, confirming mechanically that the naive analog of the existing iff would be a false statement under the corrected hypotheses, not merely an unproved one.

This is presented as an open question about the source paper (catalogued as SQ-CV-06 in `docs/specification-questions.md`), not as a correction accepted into the formalization in Sections 3–4. See `docs/laps/tangential-conversion-physical-fix/` for the full derivation and proof review.
...
Several specification ambiguities were discovered during formalization and catalogued in `docs/specification-questions.md`. Twelve remain open; five were resolved. The most consequential resolved question was the ΔP sign convention: Equation 13 of [OLENS] states `ε_d = ε'_d + ΔP`, and this sign choice is load-bearing for the `fov_undistort_eq` theorem. The specification states it consistently across Equation 13 and the inline text near Equation 10. The formal proof confirms that only this sign makes the FOV and projection forms equivalent.
...
The central pixel-agreement result of Section 4.1 — that `ws/w = fx` is required — rests on the paper's stated tangential conversion `q1 = p1/F², q2 = p2/F²`. Section 4.3 records an open question (SQ-CV-06) about whether that stated conversion is itself physically consistent with the paper's coordinate map; if the alternative derivation there is correct, the `ws/w = fx` condition would not be required at all. Readers citing the `ws/w = fx` result should be aware of this open question.
