...
**The ΔP coordinate relationship.** The specification states the projection/FOV coordinate relationship consistently: both Equation 13 and the inline text near Equation 10 write `ε_d = ε'_d + ΔP` in the same form. Formalizing this required encoding the relationship as a concrete Lean 4 function and verifying it carries through the pipeline correctly. The sign convention is load-bearing: all theorems relating the FOV and projection characterizations depend on the correct sign, and an implementation encoding the wrong direction will produce incorrect output for any lens with nonzero ΔP.
